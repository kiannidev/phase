...
use crate::game::combat::{
    band_blocker_ids, band_is_blocked, band_members, AttackTarget, CombatState, DamageAssignment,
    DamageTarget, TrampleKind,
};
...
fn band_substep_active(
    state: &GameState,
    members: &[&crate::game::combat::AttackerInfo],
    sub_step: SubStep,
    first_strike_was_done: bool,
) -> bool {
    members.iter().any(|m| {
        let Some(obj) = state.objects.get(&m.object_id) else {
            return false;
        };
        match sub_step {
            SubStep::FirstStrike => {
...
fn band_trample_kind(
    ...
    if members.iter().any(|m| {
        state
            .objects
            .get(&m.object_id)
            .is_some_and(|o| o.has_keyword(&Keyword::TrampleOverPlaneswalkers))
    }) {
        Some(TrampleKind::OverPlaneswalkers)
    } else if members.iter().any(|m| {
        state
            .objects
            .get(&m.object_id)
            .is_some_and(|o| o.has_keyword(&Keyword::Trample))
    }) {
...
fn band_total_power(state: &GameState, members: &[&crate::game::combat::AttackerInfo]) -> u32 {
    members
        .iter()
        .filter_map(|m| state.objects.get(&m.object_id))
        .map(combat_damage_amount)
        .sum()
}

fn needs_band_interactive_assignment(
    state: &GameState,
    obj: &GameObject,
    ...
    let blocker_count = blocker_ids.len();
    if obj.assigns_damage_as_though_unblocked && blocked {
    ...
    if trample.is_some() && blocker_count == 1 {
        return power > lethal_damage_needed(state, blocker_ids[0], has_deathtouch);
    }
    blocker_count >= 2
}

fn assign_band_attacker_damage(
    state: &GameState,
    ...
    if blocker_ids.is_empty() {
        ...
        let is_over_pw = trample == Some(TrampleKind::OverPlaneswalkers);
        return match attacker_info.resolve_damage_target(state, is_over_pw) {
        ...
        if let Some(trample_kind) = trample {
            ...
            if excess > 0 {
                if trample_kind == TrampleKind::OverPlaneswalkers
                    && matches!(attacker_info.attack_target, AttackTarget::Planeswalker(..))
                {
                    result.extend(assign_trample_over_pw_excess(state, attacker_info, excess));
                } else if let Some(target) = attacker_info.resolve_damage_target(state, false) {
    ...
    debug_assert!(
        false,
        "multi-blocker band auto-assignment should not be reached"
    );
    Vec::new()
}

/// CR 702.46: Assign combat damage for a band as one pooled unit.
fn collect_band_attacker_damage(
    state: &mut GameState,
    combat: &CombatState,
    band_id: u32,
    ...
    let members = band_members(combat, band_id);
    if !band_substep_active(state, &members, sub_step, first_strike_was_done) {
        return None;
    }

    let attacker_info = combat.attackers.get(first_member_index)?;
    let leader = state.objects.get(&attacker_info.object_id)?;
    let power = band_total_power(state, &members);
    if power == 0 {
        return None;
    }

    let has_deathtouch = members.iter().any(|m| {
        state
            .objects
            .get(&m.object_id)
            .is_some_and(|o| o.has_keyword(&Keyword::Deathtouch))
    });
    let trample = band_trample_kind(state, &members);
    let blocker_ids = band_blocker_ids(combat, band_id);
    let blocked = band_is_blocked(combat, band_id);

    if needs_band_interactive_assignment(
        state,
        leader,
        &blocker_ids,
        power,
        ...
        if let Some(c) = &mut state.combat {
            c.damage_step_index = Some(first_member_index);
        }
        let blockers: Vec<DamageSlot> = blocker_ids
            .iter()
            .map(|&bid| DamageSlot {
                blocker_id: bid,
            ...
            .collect();
        let assignment_modes =
            combat_damage_assignment_modes(leader, blocked, !blocker_ids.is_empty(), trample);
        let controller = leader.controller;
    ...
    let assignments = assign_band_attacker_damage(
        state,
        attacker_info,
        &blocker_ids,
        power,
    ...
    if let Some(c) = &mut state.combat {
        for assignment in assignments {
            c.pending_damage.push((attacker_info.object_id, assignment));
        }
    }
    None
}
...
fn collect_damage_assignments(state: &mut GameState, sub_step: SubStep) -> Option<WaitingFor> {
    ...
    let mut processed_bands = HashSet::new();

    // --- Attackers ---
    for (i, attacker_info) in combat.attackers.iter().enumerate().skip(start_index) {
        if let Some(band_id) = attacker_info.band_id {
            if !processed_bands.insert(band_id) {
                continue;
            }
            if let Some(waiting) = collect_band_attacker_damage(
                state,
                &combat,
                band_id,
                i,
