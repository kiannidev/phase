...
//! battlefield or stack and enter a public zone (CR 708.8).
...
fn manifested_creature_turns_face_up_in_graveyard_when_it_dies() {
    ...
    assert!(
        !obj.face_down,
        "CR 708.8: leaving the battlefield must turn the card face up in graveyard"
    );
    assert_eq!(
        obj.name, "Hidden Grizzly",
        "graveyard must show the real card name after CR 708.8 turn face up"
    );
}
