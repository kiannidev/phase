...
//! battlefield or stack and enter a public zone (CR 709.1).
...
fn manifested_creature_turns_face_up_in_graveyard_when_it_dies() {
    ...
    assert!(
        !obj.face_down,
        "CR 709.1: leaving the battlefield must turn the card face up in graveyard"
    );
    assert_eq!(
        obj.name, "Hidden Grizzly",
        "graveyard must show the real card name after CR 709.1 turn face up"
    );
}
